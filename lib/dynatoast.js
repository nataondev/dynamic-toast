class DynamicIsland {
  ...
  injectStyles() {
    ...
    style.textContent = `
                    ...
                    .di-base.idle { width: 124px; height: 38px; animation: di-float 3s ease-in-out infinite; }
                    ...
                    .di-content { opacity: 0; transition: opacity 0.2s ease; display: none; width: 100%; z-index: 10; gap: 16px; align-items: center; justify-content: space-between; }
                    ...
                    .di-close-btn { position: relative; margin-left: auto; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 20; }
  ...
  createElements() {
    ...
    this.base.onclick = () => this.handleBaseClick();
  ...
  notify({ type, title, message }) {
    ...
    // Harus sinkron: kalau ikut di dalam timeout 100ms, countdown toast
    // sebelumnya masih terlihat sejenak setelah pindah ke loading.
    this.closeBtn.style.visibility = type === "loading" ? "hidden" : "visible";
    ...
    this.notifyTimeout = setTimeout(() => {
      ...
      this.container
        .querySelector("#di-icon")
        .replaceChildren(
          new DOMParser().parseFromString(this.getIcon(type), "text/html").body
            .firstElementChild,
        );

      content.style.opacity = "1";

      if (type !== "loading") this.startCounter();
    }, 100);
